...
def calc_error_full_matrix(matrix_ratings: csr_array, mask_ratings: csr_array,
    ...
    if rev_w:
        users_reviews = mask_hidden.sum(axis=1)
        sim_matrix_hidden *= np.minimum.outer(users_reviews, users_reviews) / \
            np.maximum.outer(users_reviews, users_reviews)
        if not isinstance(sim_matrix_hidden, np.ndarray):
            print("HERE")
            sim_matrix_hidden = sim_matrix_hidden.tocsr()
...
def recalc_cos_similarity(user_ind: int, matrix_ratings, similarity_matrix: np.ndarray, users_reviews) -> None:
    """
...
def recalc_eucl_similarity(user_ind: int, matrix_ratings, mask_ratings, similarity_matrix: lil_array, users_reviews) -> None:
